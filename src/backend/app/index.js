...
router.use(cors({ origin: process.env.SERVER_CORS?.split(",") }));
...
router.get("/ai/generateText/:prompt", (req, res) =>
  aiService
    .generateText(req.params.prompt, userService.getCurrentUser(req))
...
router.post("/users/login", (req, res) => {
  userService
    ...
    .catch((error) => res.status(401).json({ error: error.message || error }));
});
