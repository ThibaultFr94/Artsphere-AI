...
function init() {
  ArtsphereApi.ai.list().then((existingImages) => {
    ...
    graff.slice(-4).forEach((element, id) => {
      document.getElementById(`g${id}`).src = element.image;
    });
    art.slice(-4).forEach((element, id) => {
      document.getElementById(`a${id}`).src = element.image;
    });
    design.slice(-4).forEach((element, id) => {
      document.getElementById(`d${id}`).src = element.image;
    });
